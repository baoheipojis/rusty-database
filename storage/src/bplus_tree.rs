...
impl BPlusTree {
    ...
    pub fn insert(&mut self, key: i32) -> Result<(), String> {
        ...
        if self.root.keys.len() == (2 * self.degree) - 1 { // 根节点已满
            // 存储旧根节点，并创建新的内部节点
...
mod tests {
    ...
    fn test_bplus_tree_search() {
        ...
        assert!(!tree.search(40)); // 检查不存在的关键字
    }

    #[test]
    fn test_bplus_tree_structure() {
        let mut tree = BPlusTree::new(2);
        tree.insert(10).unwrap();
        tree.insert(20).unwrap();
        tree.insert(30).unwrap();
